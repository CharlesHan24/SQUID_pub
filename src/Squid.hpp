...
template<int q, int _actualsize>
void Squid<q, _actualsize>::print() {
...
template<int q, int _actualsize>
void Squid<q, _actualsize>::insert(int v) {
    if (v <= _phi) {
        return;
    }
    else {
...
template<int q, int _actualsize>
void Squid<q, _actualsize>::maintenance() {
...
template<int q, int _actualsize>
int* Squid<q, _actualsize>::largestQ() {
    maintenance();
    return _A + _nminusq;
}
...
template<int q, int _actualsize>
int Squid<q, _actualsize>::PartitionAroundPivotValue(int left, int right, int pivot_val, int* nums) {
...
template<int q, int _actualsize>
int Squid<q, _actualsize>::PartitionAroundPivot(int left, int right, int pivot_idx, int* nums) {
    int pivot_value = nums[pivot_idx];
    int new_pivot_idx = right;
    swap(nums[pivot_idx], nums[right]);
...
/*
template<int q, int _actualsize>
uint32_t Squid<q, _actualsize>::mm256_extract_epi32_var_indx(int i) {
...
template<int q, int _actualsize>
int Squid<q, _actualsize>::GenerateRandom() {
    //     return std::rand() % max; 
    int indx = 0;
    if (counter >= 8) {
        rand_bits = _mm256_set_epi64x(gen_arr(), gen_arr(), gen_arr(), gen_arr());
...
*/
template<int q, int _actualsize>
void Squid<q, _actualsize>::get8samples() {
    int indx = 0;
...
int Squid<q, _actualsize>::findValueIndex(int value) {
    for (int i = 0; i < _actualsize; i++) {
        if (_A[i] == value) {
            return i;
        }
...
template<int q, int _actualsize>
int Squid<q, _actualsize>::checkPivot(int value) {
...
template<int q, int _actualsize>
int Squid<q, _actualsize>::findKthLargestAndPivot() {
    int tries = 2;
    while (tries != 0) {
...
//                 scanf("wrong %d\n",s);
    ...
    // sampling didn't work twice so we are doing the old Squid<q, _actualsize> maintanance
    int left = 0, right = _actualsizeMinusOne;
    while (left <= right) {
        int pivot_idx = left;
...
template<int q, int _actualsize>
void Squid<q, _actualsize>::reset() {
    _phi = -1;
    _curIdx = _actualsize;
}
