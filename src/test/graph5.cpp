...
void benchmark_ps_squid(key** keys, val** vals, ofstream &ostream, string dataset, int numKeys, int q, int actualSize) {
  std::random_device _rd;
  std::mt19937 _e2(_rd());
  std::uniform_real_distribution<double> _dist(0,1);
  key *elements = *keys;
...
int main() {
  ...
  for (int run = 0; run < 1; run++) {
      ...
      for (int run = 0; run < 1; run++) {
        ...
        benchmark_ps_squid(&k, &v, *stream, dataset, size, 1000000,(int) (1000000*(1+0.25)));
