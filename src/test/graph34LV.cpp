...
void benchmark_squid(int** data, ofstream& ostream) {
    int* elements = *data;
    struct timeb begintb, endtb;
    clock_t begint, endt;
    double time;
    Squid<q,actualSize> qmax;
    begint = clock();
    ftime(&begintb);
    for (int i = 0; i < N; i++) {
        qmax.insert(elements[i]);
    ...
    ftime(&endtb);
    time = ((double)(endt - begint)) / CLK_PER_SEC;
    ostream << "random,AmortizedSquidU1," << N << "," << q << "," << (double)actualSize/q-1<< "," << time << endl;
}
...
/*
      ...
      list<unsigned int> qs = {10000, 100000, 1000000, 1000};
      for (unsigned q: qs) {
...
//             benchmark_qmax(q, g, &data, ostream);
        ...
        benchmark_squid<100000,(int) (100000*(1+4))>(&data, ostream);
        benchmark_squid<100000,(int) (100000*(1+2))>(&data, ostream);
        benchmark_squid<100000,(int) (100000*(1+1))>(&data, ostream);
        benchmark_squid<100000,(int) (100000*(1+0.5))>(&data, ostream);
        benchmark_squid<100000,(int) (100000*(1+0.25))>(&data, ostream);
        benchmark_squid<100000,(int) (100000*(1+0.1))>(&data, ostream);
        benchmark_squid<100000,(int) (100000*(1+0.05))>(&data, ostream);
