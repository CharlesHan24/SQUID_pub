...
void benchmark_lrfuskiplist_g(int q, double g, double c, key** keys, ofstream &ostream, string dataset, int numKeys) {
  ...
  ftime(&endtb);
  time = ((double)(endt-begint))/CLK_PER_SEC;
  //ostream << dataset << ",SkipList_g," << numKeys << "," << q << ",," << c << "," << double(hits)/numKeys << endl;
}
...
void benchmark_lrfusquid(int q, double gamma, double c, key** keys, ofstream &ostream, string dataset, int numKeys) {
  key *elements = *keys;
  struct timeb begintb, endtb;
  clock_t begint, endt;
  double time;
  SquidIN<key, lrfu_val> squid = SquidIN<key, lrfu_val>(q, gamma, 0, lrfu_val());
  init_lrfu(c);
  begint = clock();
  ftime(&begintb);
  int hits = 0;
  for (int i = 0; i < numKeys; i++) {
    ...
    lrfu_val new_val(-i*logc);
    if(!squid.isIn(k)){
	squid.insert(k, new_val);
    } else {
	++hits;
	squid.update(k, new_val);
    }
  }
  endt = clock();
  ftime(&endtb);
  time = ((double)(endt-begint))/CLK_PER_SEC;
   //ostream << dataset << ",SQUID," << numKeys << "," << q << "," << gamma << "," << c << "," << double(hits)/numKeys << endl;
  ostream << dataset << ",SQUID," << numKeys << "," << q << "," << gamma << "," << c << "," << time << endl;
}
...
int main() {
  ...
  vector<int> sizes = {1500000};
  list<string> traces = {"P1"};
  ...
  getKeysFromFile("../datasets/CACHE/" + t + ".list", keys, CACHE_SIZE);
  ...
  for (double c: cs) {
    ...
    for (auto& stream : streams) {
      ...
      //benchmark_lrfuheap(q, c, &k, *stream, dataset, CACHE_SIZE);
	...
	//benchmark_lrfuskiplist_g(q, g, c, &k, *stream, dataset, CACHE_SIZE);
        ...
        benchmark_lrfusquid(q, g, c, &k, *stream, dataset, CACHE_SIZE);
      }
      ++k_it;
      ++d_it;
    }
