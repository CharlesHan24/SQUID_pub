...
void final_step_on_all_keys_squid(SquidIN<key, pba_val>::outputkv<key, pba_val> okv, int q) {
	for(int i = 0; i < q; ++i) {
		update_with_zstar(okv.valArr[i]);
	}
}
...
void benchmark_pbaqmax_squid(int q, double gamma, key** keys, val** ws, ofstream &ostream, string dataset, int numKeys) {
  random_device _rd;
  mt19937 _e2(_rd());
  uniform_real_distribution<double> _dist(0,1);
  key *elements = *keys;
  ...
  clock_t begint, endt;
  double time;
  SquidIN<key, pba_val> qmax = SquidIN<key, pba_val>(q, gamma, 0, pba_val());
  init_pba();//z_star = 0;
  begint = clock();
  ftime(&begintb);
  for (int i = 0; i < numKeys; i++) {
  ...
  final_step_on_all_keys_squid(qmax.largestQ(), q);
  endt = clock();
  ftime(&endtb);
  time = ((double)(endt-begint))/CLK_PER_SEC;
...
//         benchmark_pbaqmax(q, g, &k, &w, *stream, dataset, size);
        benchmark_pbaqmax_squid(q, g, &k, &w, *stream, dataset, size);
      }
      ++k_it;
      ++w_it;
      ++s_it;
