...
void benchmark_hh_squid(key** keys, ofstream &ostream, string dataset, int numKeys, int q, int actualSize) {
  key *elements = *keys;
  struct timeb begintb, endtb;
  clock_t begint, endt;
  double time;
  SquidKV lvqmax = SquidKV(q,actualSize);
  begint = clock();
  ftime(&begintb);
  for (int i = 0; i < numKeys; i++) {
    val hash = XXH64((void *) &(elements[i]), sizeof(key), 1235);
...
int main() {
  ...
  for (int run = 0; run < 2; run++) {
    ...
    for (auto& stream : streams) {
        ...
        benchmark_hh_squid(&k, *stream, dataset, size, 1000000,(int) (1000000*(1+0.25)));

//         benchmark_hh_squid<1000000,(int) (1000000*(1+0.1))>(&k, *stream, dataset, size);
// 
//         benchmark_hh_squid<1000000,(int) (1000000*(1+0.05))>(&k, *stream, dataset, size);
// 
//         benchmark_hh_squid<10000000,(int) (10000000*(1+0.25))>(&k, *stream, dataset, size);
// 
//         benchmark_hh_squid<10000000,(int) (10000000*(1+0.1))>(&k, *stream, dataset, size);
// 
//         benchmark_hh_squid<10000000,(int) (10000000*(1+0.05))>(&k, *stream, dataset, size);
//         
