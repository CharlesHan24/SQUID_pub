...
SquidKV::SquidKV(int q1, int actualsize) {
    q=q1;
...
void SquidKV::insert(key k, val v) {
//     printf("in2\n");
    if (v < _phi) {
        return;
    }
...
void SquidKV::maintenance() {
    _phi = findKthLargestAndPivot();
}


outputkv SquidKV::largestQ() {
	outputkv out;
	maintenance();
...
int SquidKV::PartitionAroundPivot(int left, int right, int pivot_idx, val* nums) {
...
int SquidKV::findValueIndex(val value) {
    for (int i = 0; i < _actualsize; i++) {
        if (_V[i] == value) {
            return i;
        }
...
// if value dont exist in _K return -1, otherwise return the index of the value in _K
int SquidKV::findValueDup(val value,int idx, int check) {
    int sum=check;
...
int SquidKV::checkPivot(val value) {
...
val SquidKV::findKthLargestAndPivot() {
    int tries = 2;
    while (tries != 0) {
...
//     printf("fail\n");
    ...
    // sampling didn't work twice so we are doing the old SquidKV maintanance
    int left = 0, right = _actualsizeMinusOne;
	while (left <= right) {
