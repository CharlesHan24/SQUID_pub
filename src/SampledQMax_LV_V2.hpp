...
template<int q, int _actualsize>
void SampledQMax_LV_V2<q, _actualsize>::print() {
...
template<int q, int _actualsize>
SampledQMax_LV_V2<q, _actualsize>::SampledQMax_LV_V2() {
...
template<int q, int _actualsize>
void SampledQMax_LV_V2<q, _actualsize>::insert(int v) {
    if (v <= _phi) {
        return;
    }
    else {
...
template<int q, int _actualsize>
void SampledQMax_LV_V2<q, _actualsize>::maintenance() {
...
//         endt = clock();
//         ftime(&endtb);
//         double t = ((double)(endt - begint)) / CLK_PER_SEC;
//         
//         
//         
//         printf("lv time%f\n",t);
...
template<int q, int _actualsize>
int* SampledQMax_LV_V2<q, _actualsize>::largestQ() {
    maintenance();
    return _A + _nminusq;
}
...
template<int q, int _actualsize>
int SampledQMax_LV_V2<q, _actualsize>::PartitionAroundPivotValue(int left, int right, int pivot_val, int* nums) {
...
template<int q, int _actualsize>
int SampledQMax_LV_V2<q, _actualsize>::PartitionAroundPivot(int left, int right, int pivot_idx, int* nums) {
    int pivot_value = nums[pivot_idx];
    int new_pivot_idx = right;
    swap(nums[pivot_idx], nums[right]);
...
/*
template<int q, int _actualsize>
uint32_t SampledQMax_LV_V2<q, _actualsize>::mm256_extract_epi32_var_indx(int i) {
...
*/
template<int q, int _actualsize>
void SampledQMax_LV_V2<q, _actualsize>::get8samples() {
    int indx = 0;
...
int SampledQMax_LV_V2<q, _actualsize>::findValueIndex(int value) {
    for (int i = 0; i < _actualsize; i++) {
        if (_A[i] == value) {
            return i;
        }
...
template<int q, int _actualsize>
int SampledQMax_LV_V2<q, _actualsize>::checkPivot(int value) {
...
template<int q, int _actualsize>
int SampledQMax_LV_V2<q, _actualsize>::findKthLargestAndPivot() {
    int tries = 2;
    while (tries != 0) {
...
template<int q, int _actualsize>
void SampledQMax_LV_V2<q, _actualsize>::reset() {
    _phi = -1;
    _curIdx = _actualsize;
}
