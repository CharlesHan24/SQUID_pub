...
for l in [0.1]:
    if G == "CPU":
	q_max_filter_string='AmortizedSampledQMax_LV_V2U1'
    else:
	q_max_filter_string='QMax-LV'
    ...
    ax1.errorbar(sizes, ga['mean'], yerr=ga['std'], label=r'SQUID,$\gamma=$' + str(l), markersize=10, marker=next(mark), fmt='o', linewidth=2, capsize=10, elinewidth=3, capthick=1.5)
...
for l in [0.1]:
    ...
    if l < 0.25:
        ...
        ax1.errorbar(sizes, ga['mean'], yerr=ga['std'], label=r'$q$-MAX,$\gamma=$' + str(l), markersize=10, marker='H', color='black', fmt='o', linewidth=2, capsize=10, elinewidth=3, capthick=1.5)

handles, labels = ax1.get_legend_handles_labels()
ax1.legend(handles, labels, loc='lower left',ncol=1, prop={'size':15})
