...
for s in sizes:
	...
	df_org['type'] = df_org.apply(lambda row: r'SQUID,$\gamma=$' + str(row['gamma']) if row['type'] == 'LVAmortizedQMax' else row['type'], axis=1)
...
#fig, ax = plt.subplots(1,1)  
#bar_cycle = cycler(color=list('rymkwcb')) + cycler(hatch=[ 'xX', '\\\\', '**', '..', 'OO', '--','--'])
#styles = bar_cycle()
#ax.set_ylabel('Throughput [MPPS]', fontsize=20)
#ax.yaxis.grid(True)
...
#for s in sizes:
	...
	#df_org['type'] = df_org.apply(lambda row: r'$q$-Sized LRFU'  if row['type'] == 'Heap' else row['type'], axis=1)
	...
	#w=-0.6
	#i=0
	#for c in [r'$q$-MAX,$\gamma=$0.05' , r'SQUID,$\gamma=$0.05' ,  r'$q$-MAX,$\gamma=$0.1' , r'SQUID,$\gamma=$0.1', r'$q$-MAX,$\gamma=$0.25'  , r'SQUID,$\gamma=$0.25', r'$q$-Sized LRFU']:
	    #y = []
	    #yerr = []
	    #for t in traces:
	        #y.append(j[c][t])
