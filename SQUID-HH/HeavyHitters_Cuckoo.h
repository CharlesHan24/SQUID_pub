...
//     max_load = max_load/(1+gamma);
    ...
    _phase = 0;
    _delta_org = 0.05;
    _delta = _delta_org/(2*_M0);
    _maintenances_Until_delta_update = _M0;
...
//             cout << smpl << endl;
    ...
    if (--_maintenances_Until_delta_update==0){
            _phase +=1;
            _maintenances_Until_delta_update = _M0 * (1<<(_phase-1));
            
            _delta= _delta_org *pow(4,-_phase) / _M0;
            updateZK();
    }
