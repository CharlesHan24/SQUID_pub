...
double run(int traceSize, key* kk, val* vv, int runs, double maxl, double gamma, int bins){
    double time = 0;
    for (int run = 0; run < runs; run++) {
            ...
            clock_t begint, endt;
            Cuckoo_waterLevel_HH_no_FP_SIMD_256<key, val> hh(1, 2, 3, maxl,gamma, bins);
            begint = clock();
...
double runNrmse(int traceSize, key* kk, val* vv, int runs, double maxl, double gamma, int bins){
    float sum=0.0;
    for (int run = 0; run < runs; run++) {
        double c =0; 
        uint64_t vol =0;
...
double t = 0 ;
 t = run(size, kk , vv,k, maxl, gamma, 1 <<18);
float nrmse = 0;
