...
#define COLUMN_CNT 12 // upscaled to power of 2
#define ARRAY_BITWIDTH 16
#define COLUMN_BITWIDTH 4
...
#define SAMPLE_COL_THRES1 5461
#define SAMPLE_COL_THRES2 10922
#define SAMPLE_COL_THRES3 16384
#define SAMPLE_COL_THRES4 21845
#define SAMPLE_COL_THRES5 27306
#define SAMPLE_COL_THRES6 32768
#define SAMPLE_COL_THRES7 38229
#define SAMPLE_COL_THRES8 43690
#define SAMPLE_COL_THRES9 49152
#define SAMPLE_COL_THRES10 54613
#define SAMPLE_COL_THRES11 60074
...
control SwitchIngress(inout header_t hdr,
    ...
    Random<bit<12> >() need_sample_random;
    ...
    RegisterParam<int<32> >(0) pmax_array_threshold## id ##; \
    Register<item_t, bit<16> >(ARRAY_LEN) pmax_array## id ##; \
    RegisterAction<item_t, bit<16>, int<32> >(pmax_array## id ##) pmax_array## id ##_insert_action = { \
        void apply(inout item_t value, out int<32> read_value) { \
            if ((value.key == ig_md.qmax_array_md1) || (value.score < pmax_array_threshold## id ##.read())) { \
                read_value = 1; \
                value.key = ig_md.qmax_array_md1; \
                value.score = ig_md.qmax_array_md2; \
            } \
    ...
    RegisterAction<item_t, bit<16>, int<32> >(pmax_array## id ##) pmax_array## id ##_query_score_action = { \
        void apply(inout item_t value, out int<32> read_value) { \
            read_value = value.score; \
        } \
    }; \
    RegisterAction<item_t, bit<16>, int<32> >(pmax_array## id ##) pmax_array## id ##_lookup_update_action = { \
        void apply(inout item_t value, out int<32> read_value) { \
            if (value.key == ig_md.qmax_array_md1) { \
                read_value = value.score; \
                if (value.score - ig_md.qmax_array_md2 < -5) { \
    ...
    action get_const_ipv4_action## id ##(bit<32> threshold, PortId_t port) { \
        ...
        ig_md.is_sample[11:0] = need_sample_random.get(); \
    ...
    action init_sample_column_action(bit<32> res_column){
        hdr.ig_eg_info.sample_column = res_column;
        hdr.control_hdr.proto = CTRL_REINSERT_SAMPLE;
    }
    table init_sample_column_tab {
        key = {
            ig_intr_md.resubmit_flag: exact;
            ig_md.sample_column: ternary;
        }
        actions = {
            init_sample_column_action;
        }
        size = 32;
        default_action = init_sample_column_action(0);
    }
    ...
    apply {
        ...
        if (ig_intr_md.resubmit_flag == 1) {
            ...
            if (hdr.ig_eg_info.sample_column == 0){
                hdr.cpu.score = qmax_array1_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 1){
                hdr.cpu.score = pmax_array1_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 2){
                hdr.cpu.score = qmax_array2_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 3){
                hdr.cpu.score = pmax_array2_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 4){
                hdr.cpu.score = qmax_array3_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 5){
                hdr.cpu.score = pmax_array3_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 6){
                hdr.cpu.score = qmax_array4_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 7){
                hdr.cpu.score = pmax_array4_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 8){
                hdr.cpu.score = qmax_array5_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 9){
                hdr.cpu.score = pmax_array5_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 10){
                hdr.cpu.score = qmax_array6_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 11){
                hdr.cpu.score = pmax_array6_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 12){
                hdr.cpu.score = qmax_array7_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 13){
                hdr.cpu.score = pmax_array7_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 14){
                hdr.cpu.score = qmax_array8_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 15){
                hdr.cpu.score = pmax_array8_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 16){
                hdr.cpu.score = qmax_array9_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 17){
                hdr.cpu.score = pmax_array9_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 18){
                hdr.cpu.score = qmax_array10_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 19){
                hdr.cpu.score = pmax_array10_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 20){
                hdr.cpu.score = qmax_array11_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 21){
                hdr.cpu.score = pmax_array11_query_score_action.execute(ig_md.sample_idx);
            }
            else if (hdr.ig_eg_info.sample_column == 22){
                hdr.cpu.score = qmax_array12_query_score_action.execute(ig_md.sample_idx);
            }
            else{
                hdr.cpu.score = pmax_array12_query_score_action.execute(ig_md.sample_idx);
            }
        }
            ...
            if (hdr.control_hdr.proto == CTRL_INBOUND) {
                ...
                if (ig_md.is_sample[0:0] == 0){
                    ...
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array5_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 4;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array6_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 5;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array7_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 6;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array8_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 7;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array9_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 8;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array10_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 9;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array11_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 10;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array12_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 11;
                    }
                    if ((ig_md.cur_res != 0) && (hdr.ig_eg_info.affected_col == 0)){
                        hdr.ig_eg_info.affected_col = 12;
                    }
                }
                else{
                    ig_md.cur_res = pmax_array1_lookup_update_action.execute(hdr.ig_eg_info.index);
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array2_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 13;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array3_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 14;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array4_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 15;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array5_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 16;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array6_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 17;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array7_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 18;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array8_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 19;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array9_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 20;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array10_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 21;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array11_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 22;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array12_lookup_update_action.execute(hdr.ig_eg_info.index);
                    }
                    else{
                        hdr.ig_eg_info.affected_col = 23;
                    }

                    if ((ig_md.cur_res != 0) && (hdr.ig_eg_info.affected_col == 0)){
                        hdr.ig_eg_info.affected_col = 24;
                    }
                }
                ...
                if (ig_md.is_sample[0:0] == 0){
                    ...
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array5_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 4;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array6_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 5;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array7_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 6;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array8_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 7;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array9_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 8;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array10_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 9;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array11_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 10;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = qmax_array12_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 11;
                    }
                    ...
                    ig_md.cur_res = pmax_array1_insert_action.execute(hdr.ig_eg_info.index);
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array2_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 13;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array3_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 14;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array4_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 15;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array5_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 16;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array6_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 17;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array7_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 18;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array8_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 19;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array9_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 20;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array10_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 21;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array11_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 22;
                    }
                    if (ig_md.cur_res == 0){
                        ig_md.cur_res = pmax_array12_insert_action.execute(hdr.ig_eg_info.index);
                    }
                    else if (hdr.ig_eg_info.affected_col == 0){
                        hdr.ig_eg_info.affected_col = 23;
                    }
...
control SwitchEgress(
    ...
    apply {
        ...
        if (hdr.control_hdr.proto == CTRL_REINSERT_SAMPLE) {
            ...
            if ((hdr.ig_eg_info.affected_col == 1) || (hdr.ig_eg_info.affected_col == 2)){
            ...
            else if ((hdr.ig_eg_info.affected_col == 5) || (hdr.ig_eg_info.affected_col == 6)){
                hdr.flow_info.val = value_cache3_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 7) || (hdr.ig_eg_info.affected_col == 8)){
                hdr.flow_info.val = value_cache4_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 9) || (hdr.ig_eg_info.affected_col == 10)){
                hdr.flow_info.val = value_cache5_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 11) || (hdr.ig_eg_info.affected_col == 12)){
                hdr.flow_info.val = value_cache6_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 13) || (hdr.ig_eg_info.affected_col == 14)){
                hdr.flow_info.val = value_cache7_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 15) || (hdr.ig_eg_info.affected_col == 16)){
                hdr.flow_info.val = value_cache8_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 17) || (hdr.ig_eg_info.affected_col == 18)){
                hdr.flow_info.val = value_cache9_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 19) || (hdr.ig_eg_info.affected_col == 19)){
                hdr.flow_info.val = value_cache10_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 21) || (hdr.ig_eg_info.affected_col == 22)){
                hdr.flow_info.val = value_cache11_insertquery_action.execute(eg_md.index1);
            }
            else if ((hdr.ig_eg_info.affected_col == 23) || (hdr.ig_eg_info.affected_col == 24)){
                hdr.flow_info.val = value_cache12_insertquery_action.execute(eg_md.index1);
            }
